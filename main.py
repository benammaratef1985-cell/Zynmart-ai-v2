...
def get_gemini_vision_response(user_message, image_bytes, mime, user_name=""):
    """Analyze one user image with Gemini. Uses one key, then one alternate only after rate/auth failure."""
    if not GEMINI_API_KEYS or not image_bytes or mime not in ("image/jpeg","image/png","image/webp"):
        return None
    context_text = _build_ai_context(user_message, "")
    prompt = (ZYNMART_PROMPT + "\nحلّل الصورة المرفقة بدقة. إذا كان السؤال عن لون أو نص أو عنصر ظاهر في الصورة، صف ما تراه فقط ولا تخمّن.\n" + context_text[:1200] + f"\n\nالمستخدم ({user_name}): {user_message}")
    ...
    for _ in range(min(len(GEMINI_API_KEYS),2)):
        ...
        attempted.add(idx)
        try:
            res=requests.post("https://generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash:generateContent?key="+k,json=payload,headers={"Content-Type":"application/json"},timeout=15)
            if res.status_code==200:
                parts=res.json().get("candidates",[{}])[0].get("content",{}).get("parts",[])
                txt=parts[0].get("text","") if parts else ""
                if txt:return sanitize_urls(txt)
                return None
            ...
            print(f"Gemini Vision Exception: {e} - key index {idx}")
            return None
    return None
...
PLATFORM_SECTION_META = {
    ...
    "stores": ("🖼️", "Marketplace ZynMart NFT", "سوق ZynMart لعرض واستكشاف أصول NFT من AI for — المجموعات والعروض وGenesis"),
...
PLATFORM_STATUS = {
    ...
    "stores": {"active": ["عرض مجموعات NFT المنشورة", "استكشاف الأصول والـMetadata", "IPFS وروابط الأصول", "Genesis الخاصة بالمنصة", "تكامل AI for داخل Marketplace"], "soon": ["Minting على Pi/Soroban", "البيع والمعاملات", "السوق التجاري الكامل للتجار"]},
...
def _nft_marketplace_list(user, limit=60):
    """Public NFT marketplace feed. Only explicitly listed/published IPFS-ready projects are exposed."""
    conn = None
    try:
        if not membership_db_ready:
            return {"ok": False, "error": "database_unavailable", "items": []}
        conn = _membership_db_connect()
        if not conn:
            return {"ok": False, "error": "database_unavailable", "items": []}
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute("""SELECT project_id,name,description,collection_name,status,updated_at,ipfs_cid,ipfs_uri,marketplace_url,contract_id,contract_network,genesis_enabled,genesis_benefits,metadata
                           FROM ai_for_nft_projects
                           WHERE status IN ('listed','published','minted') OR (status='ipfs_pinned' AND ipfs_cid IS NOT NULL AND ipfs_cid <> '')
                           ORDER BY updated_at DESC LIMIT %s""", (max(1, min(int(limit), 100)),))
            rows = cur.fetchall()
        items=[]
        for r in rows:
            items.append({
                "project_id": str(r["project_id"]), "name": r["name"], "description": r["description"],
                "collection_name": r["collection_name"], "status": r["status"],
                "updated_at": r["updated_at"].isoformat() if r.get("updated_at") else None,
                "ipfs_cid": r.get("ipfs_cid") or "", "ipfs_uri": r.get("ipfs_uri") or "",
                "marketplace_url": r.get("marketplace_url") or "", "contract_id": r.get("contract_id") or "",
                "contract_network": r.get("contract_network") or "", "genesis_enabled": bool(r.get("genesis_enabled")),
                "genesis_benefits": r.get("genesis_benefits") or "",
                "metadata": r.get("metadata") if isinstance(r.get("metadata"), dict) else {}
            })
        return {"ok": True, "items": items, "marketplace": "Marketplace ZynMart NFT"}
    except Exception as e:
        print(f"NFT marketplace list error: {e}")
        return {"ok": False, "error": "marketplace_load_failed", "items": []}
    finally:
        _membership_db_release(conn)

def _nft_marketplace_list_project(user, project_id):
    if not (_webapp_is_owner(user) or _webapp_is_admin(user)):
        return False
    conn=None
    try:
        conn=_membership_db_connect()
        ...
        with conn:
            with conn.cursor() as cur:
                cur.execute("UPDATE ai_for_nft_projects SET status='listed', marketplace_url=%s, updated_at=NOW() WHERE project_id=%s", (NFT_MARKETPLACE_URL or 'ai-for://marketplace-zynmart-nft', project_id))
                return cur.rowcount == 1
    except Exception as e:
        print(f"NFT marketplace list project error: {e}")
        return False
    finally:
        _membership_db_release(conn)
...
async function renderZynMartNFTMarketplace(){
 document.getElementById('view').innerHTML=`<button class="back" onclick="goHome()">← المنصة</button><section class="detail"><div class="sectionTitle">🖼️ Marketplace ZynMart NFT</div><p class="small">سوق NFT الخاص بـZynMart داخل AI for — عرض الأصول المنشورة وMetadata وIPFS وGenesis مع تكامل هوية AI for.</p><div class="toolbar"><button class="action" onclick="loadZynMartNFTMarketplace()">🔄 تحديث</button><button class="action" onclick="openSection('ai')">🤖 مساعد AI for</button></div><div id="zynNFTMarket" class="grid"><div class="statusBox">⏳ جاري تحميل Marketplace...</div></div></section>`;loadZynMartNFTMarketplace()}
async function loadZynMartNFTMarketplace(){let r=document.getElementById('zynNFTMarket');if(!r)return;r.innerHTML='<div class="statusBox">⏳ جاري تحميل الأصول...</div>';try{let d=await api('/api/app/marketplace/nft');if(!d.ok)throw new Error(d.error||'marketplace_load_failed');let items=d.items||[];r.innerHTML=items.length?items.map(p=>{let md=p.metadata||{};let image=md.image||md.image_url||md.image_uri||'';return `<article class="card nftMarketCard">${image?`<img src="${esc(image)}" alt="" style="width:100%;aspect-ratio:1/1;object-fit:cover;border-radius:18px;margin-bottom:10px">`:'<div class="ico">🖼️</div>'}<h3>${esc(p.name||'NFT')}</h3><p>${esc(p.collection_name||'AI for Collection')}</p><p class="small">${esc(p.description||md.description||'')}</p><div class="small">${p.ipfs_cid?'📌 IPFS: '+esc(p.ipfs_cid):''}${p.genesis_enabled?' · 🧬 Genesis':''}</div>${p.ipfs_uri?`<a class="badge external" href="${esc(p.ipfs_uri)}" target="_blank" rel="noopener noreferrer">↗ Metadata</a>`:''}</article>`}).join(''):'<div class="statusBox">لا توجد أصول منشورة في Marketplace حاليًا.</div>'}catch(e){r.innerHTML='<div class="statusBox">⚠️ تعذر تحميل Marketplace الآن.</div>'}}
...
async function listCurrentNFTOnMarketplace(){let m=document.getElementById('nftMsg'),id=window.__AI_FOR_NFT_PROJECT_ID||'';if(!id){m.textContent='⚠️ احفظ المشروع أولًا.';return}try{let d=await api('/api/app/marketplace/nft/list',{method:'POST',body:JSON.stringify({project_id:id})});if(!d.ok)throw new Error(d.error||'list_failed');m.innerHTML='✅ تم إدراج المشروع في Marketplace ZynMart NFT.';loadNFTProjects()}catch(e){m.textContent='⚠️ '+(e.message||'تعذر الإدراج')}}
...
def webapp_ai():
    ...
    if denied[0]: return denied
    if emergency_active(): return jsonify({"ok": False, "text": "⚠️ وضع الطوارئ مفعّل حاليًا."}), 503
    body = request.get_json(silent=True) or {} if request.mimetype != "multipart/form-data" else request.form.to_dict()
    ...
    if uploaded_image and (not image_bytes or image_mime not in ("image/jpeg","image/png","image/webp")):
        return jsonify({"ok":False,"text":"⚠️ صورة غير صالحة. استخدم JPG أو PNG أو WebP حتى 2MB."}),400
    ...
    try:
        if image_bytes:
            ...
            if reply:
                history = _webapp_history(user, conversation_id); history.extend([{"role":"user","text":question or "[صورة]"},{"role":"assistant","text":reply}]); _webapp_save_history(user, conversation_id, history)
        else:
...
def webapp_account_photo_get():
    user, err, code=_webapp_auth()
    if err: return err, code
    row=get_web_profile_photo(user.get("platform_account_id")) if user.get("auth_type") == "web" else get_profile_photo(user.get("id"))
...
@app.route("/api/app/marketplace/nft", methods=["GET"])
def webapp_nft_marketplace():
    user, err, code = _webapp_auth()
    if err: return err, code
    denied = _webapp_require_section(user, "stores")
    if denied[0]: return denied
    return jsonify(_nft_marketplace_list(user))

@app.route("/api/app/marketplace/nft/list", methods=["POST"])
def webapp_nft_marketplace_list():
    user, err, code = _webapp_auth()
    if err: return err, code
    ...
    if not _nft_marketplace_list_project(user, project_id):return jsonify({"ok":False,"error":"list_failed"}),404
    control_audit(user["id"], "nft_marketplace_list", project_id, {"marketplace":"Marketplace ZynMart NFT"})
    return jsonify({"ok":True,"status":"listed","marketplace":"Marketplace ZynMart NFT"})
...
def owner_feature_tests(user):
    ...
    required_routes={
        ...
        "webapp_nft_marketplace":"/api/app/marketplace/nft",
        "webapp_nft_marketplace_list":"/api/app/marketplace/nft/list"
    }
    ...
    add("zynmart_nft_marketplace", callable(globals().get("_nft_marketplace_list")) and "webapp_nft_marketplace" in app.view_functions, "Marketplace ZynMart NFT exposes only listed/IPFS-ready NFT assets")
